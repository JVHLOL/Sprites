...
function setupUI() {
  ...
  document.getElementById('btn-copy-discord').addEventListener('click', async () => {
    ...
    const stats = computeStats();
    const lines = [
      `**Sprite Locker** — ${stats.owned}/${stats.total} (${stats.total ? Math.round(stats.owned/stats.total*100) : 0}%)`,
      `Mastered: ${stats.mastered}`,
      `Rare ${stats.byRarity.rare.o}/${stats.byRarity.rare.t} · Epic ${stats.byRarity.epic.o}/${stats.byRarity.epic.t} · Legendary ${stats.byRarity.legendary.o}/${stats.byRarity.legendary.t} · Mythic ${stats.byRarity.mythic.o}/${stats.byRarity.mythic.t}`,
      'https://jvhlol.github.io/Sprites/'
    ];
    try {
      await navigator.clipboard.writeText(lines.join('\n'));
      moreBtn.innerHTML = '<span class="more-dots">✓</span> COPIED';
      setTimeout(() => { moreBtn.innerHTML = '<span class="more-dots">⋯</span> MORE <span class="more-caret">▾</span>'; }, 1500);
    } catch { alert('Could not copy — allow clipboard access.'); }
  });
