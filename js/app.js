...
function buildShareCanvas() {
  ...
  bg.addColorStop(0.5, '#12080c');
  bg.addColorStop(1, '#0e0608');
  ctx.fillStyle = bg;
  ctx.fillRect(0, 0, W, H);

  const glow = ctx.createRadialGradient(W * 0.2, 0, 0, W * 0.2, 0, 280);
  glow.addColorStop(0, 'rgba(120, 20, 40, 0.45)');
  ...
  ctx.fillText('SPRITE LOCKER', pad, 42);
  ctx.fillStyle = '#c4a0a8';
  ctx.font = '600 13px Inter, system-ui, sans-serif';
  ctx.fillText('Fortnite Sprite Collection', pad, 64);
  ...
  ctx.fillText(`${pct}%`, pad + 200, 112);

  const barX = pad, barY = 128, barW = W - pad * 2, barH = 12;
  roundRect(ctx, barX, barY, barW, barH, 6);
  ctx.fillStyle = 'rgba(0,0,0,0.4)';
  ctx.fill();
  if (pct > 0) {
    roundRect(ctx, barX, barY, Math.max(8, barW * pct / 100), barH, 6);
  ...
  rarities.forEach((r, i) => {
    ...
    const y = headerH + 8;
    roundRect(ctx, x, y, cardW, 88, 12);
    ctx.fillStyle = 'rgba(40, 12, 18, 0.7)';
    ctx.fill();
    ctx.strokeStyle = 'rgba(255, 120, 140, 0.25)';
    ctx.lineWidth = 1;
    ctx.stroke();
    ctx.beginPath();
    ...
    ctx.fillText(r.label, x + 28, y + 26);
    const { o, t } = stats.byRarity[r.key];
    ctx.fillStyle = '#f1e8ea';
    ctx.font = '800 22px Inter, system-ui, sans-serif';
    ctx.fillText(`${o}/${t}`, x + 14, y + 56);
    const rp = t ? o / t : 0;
    roundRect(ctx, x + 14, y + 68, cardW - 28, 6, 3);
    ctx.fillStyle = 'rgba(0,0,0,0.35)';
    ctx.fill();
    if (rp > 0) {
      roundRect(ctx, x + 14, y + 68, Math.max(4, (cardW - 28) * rp), 6, 3);
      ctx.fillStyle = r.color;
      ctx.fill();
    }
  });
  ...
  ctx.fillText(`Mastered  ${stats.mastered}`, W - pad - 120, 42);
  ...
  ctx.fillText('jvhlol.github.io/Sprites  ·  fan tracker', pad, H - 18);
...
function setupUI() {
  ...
  document.getElementById('btn-copy-discord').addEventListener('click', async () => {
    closeMore();
    moreBtn.innerHTML = '<span class="more-dots">…</span> MAKING';
    try {
      const ok = await copyShareImage();
      moreBtn.innerHTML = ok
        ? '<span class="more-dots">✓</span> IMAGE COPIED'
        : '<span class="more-dots">↓</span> DOWNLOADED';
    } catch (e) {
      console.error(e);
