...
function buildShareCanvas() {
  ...
  const headerH = 120, rarityH = 100, gridTop = headerH + rarityH + 20;
  const H = gridTop + SPRITES.length * (cell + gap) + 50;
  const canvas = document.createElement('canvas');
  canvas.width = W; canvas.height = H;
  const ctx = canvas.getContext('2d');
  const bg = ctx.createLinearGradient(0, 0, 0, H);
  bg.addColorStop(0, '#1e5bb8'); bg.addColorStop(0.4, '#1a4fa0'); bg.addColorStop(1, '#0f2d6b');
  ctx.fillStyle = bg; ctx.fillRect(0, 0, W, H);
  ctx.fillStyle = 'rgba(255,255,255,0.55)'; ctx.font = '600 11px Inter, system-ui, sans-serif';
  ...
  ctx.fillStyle = 'rgba(255,255,255,0.7)'; ctx.font = '500 12px Inter, system-ui, sans-serif';
  ctx.fillText('Have vs. need — open to trade', pad, 78);
  ctx.fillStyle = '#f6c343'; ctx.font = '800 32px Inter, system-ui, sans-serif';
  ctx.fillText(`${stats.owned} / ${stats.total}`, pad, 112);
  ctx.fillStyle = '#fff'; ctx.font = '700 18px Inter, system-ui, sans-serif';
  ctx.fillText(`${pct}%`, pad + 180, 112);
  roundRect(ctx, pad, 122, W - pad * 2, 10, 5);
  ctx.fillStyle = 'rgba(0,0,0,0.3)'; ctx.fill();
  if (pct > 0) { roundRect(ctx, pad, 122, Math.max(8, (W - pad * 2) * pct / 100), 10, 5); ctx.fillStyle = '#38bdf8'; ctx.fill(); }
  const rarities = [{ key: 'rare', label: 'Rare', color: '#38bdf8' }, { key: 'epic', label: 'Epic', color: '#c084fc' }, { key: 'legendary', label: 'Legendary', color: '#fbbf24' }, { key: 'mythic', label: 'Mythic', color: '#f87171' }];
  const cardW = (W - pad * 2 - 18) / 4;
  rarities.forEach((r, i) => {
    const x = pad + i * (cardW + 6), y = headerH + 8;
    roundRect(ctx, x, y, cardW, 80, 12); ctx.fillStyle = 'rgba(0,20,60,0.45)'; ctx.fill();
    ctx.beginPath(); ctx.arc(x + 16, y + 20, 5, 0, Math.PI * 2); ctx.fillStyle = r.color; ctx.fill();
    ctx.fillStyle = 'rgba(255,255,255,0.8)'; ctx.font = '600 11px Inter, system-ui, sans-serif'; ctx.fillText(r.label, x + 28, y + 24);
    const { o, t } = stats.byRarity[r.key];
    ctx.fillStyle = '#fff'; ctx.font = '800 20px Inter, system-ui, sans-serif'; ctx.fillText(`${o}/${t}`, x + 14, y + 52);
    const rp = t ? o / t : 0;
    roundRect(ctx, x + 14, y + 62, cardW - 28, 6, 3); ctx.fillStyle = 'rgba(0,0,0,0.35)'; ctx.fill();
    if (rp > 0) { roundRect(ctx, x + 14, y + 62, Math.max(4, (cardW - 28) * rp), 6, 3); ctx.fillStyle = r.color; ctx.fill(); }
  });
  ctx.fillStyle = '#f6c343'; ctx.font = '700 12px Inter, system-ui, sans-serif';
  ctx.fillText(`Mastered ${stats.mastered}`, W - pad - 110, 40);
  const gridX = pad + nameW, gridY = gridTop;
  VARIANTS.forEach((v, i) => { const x = gridX + i * (cell + gap); ctx.fillStyle = VARIANT_META[v].color; roundRect(ctx, x, gridY - 18, cell, 12, 3); ctx.fill(); });
  ctx.fillStyle = 'rgba(255,255,255,0.7)'; ctx.font = '600 10px Inter, system-ui, sans-serif'; ctx.fillText('SPRITE', pad, gridY - 8);
  SPRITES.forEach((sp, ri) => {
    const y = gridY + ri * (cell + gap);
    ctx.fillStyle = RARITY_COLORS[sp.rarity] || '#fff'; ctx.beginPath(); ctx.arc(pad + 6, y + cell / 2, 4, 0, Math.PI * 2); ctx.fill();
    ...
    VARIANTS.forEach((v, vi) => {
      const st = getStatus(sp.id, v), x = gridX + vi * (cell + gap);
      if (st === 'na') ctx.fillStyle = 'rgba(71,85,105,0.35)';
      else if (st === 'soon') ctx.fillStyle = 'rgba(109,40,217,0.55)';
      ...
      if (st === 'mastered') { ctx.fillStyle = '#0b1220'; ctx.font = '700 9px Inter, system-ui, sans-serif'; ctx.fillText('*', x + 4, y + 11); }
      if (st === 'soon') { ctx.fillStyle = '#e9d5ff'; ctx.font = '700 7px Inter, system-ui, sans-serif'; ctx.fillText('S', x + 4, y + 11); }
    });
  });
  ctx.fillStyle = 'rgba(255,255,255,0.45)'; ctx.font = '500 11px Inter, system-ui, sans-serif';
