...
function openModal(id, variant) {
  ...
  const imgHtml = img ? `<img class="modal-img" src="${img}" alt=""/>` : `<span style="font-size:2rem">${emojiFor(id)}</span>`;
  backdrop.innerHTML = `
    <div class="sprite-modal" role="dialog">
      ...
      <div class="modal-title-row">
        ...
        <div>
          ...
          <div class="modal-badges">
            <span class="modal-badge rarity-${sp.rarity}">${sp.rarity}</span>
            <span class="modal-badge variant">${meta.label}</span>
          </div>
        </div>
      </div>
      <div class="modal-section-label">Effect</div>
      <p class="modal-effect">${sp.ability || ''}</p>
      <p class="modal-bonus"><strong>${meta.label} bonus:</strong> ${meta.desc}</p>
      ${sp.where ? `<p class="modal-bonus" style="margin-top:4px"><strong>Where:</strong> ${sp.where}</p>` : ''}
      <div class="modal-actions">
        <button type="button" data-act="owned" class="${st === 'owned' ? 'active-owned' : ''}"><span>✓</span> Owned</button>
        <button type="button" data-act="mastered" class="${st === 'mastered' ? 'active-mastered' : ''}"><span>👑</span> Mastered</button>
        <button type="button" data-act="lost" class="${st === 'lost' ? 'active-lost' : ''}"><span>⊘</span> Lost</button>
      </div>
      <div class="modal-level">
        <span class="label">Level</span>
        <div class="controls">
          <button type="button" data-lvl="-">−</button>
          <span class="level-val">${level} / 5</span>
          <button type="button" data-lvl="+">+</button>
        </div>
      </div>
    </div>`;
  ...
  backdrop.querySelectorAll('[data-lvl]').forEach(btn => {
    btn.onclick = () => {
      const d = btn.dataset.lvl === '+' ? 1 : -1;
      let st = getStatus(id, variant);
      if (st == null || st === 'lost') setStatus(id, variant, 'owned');
...
function render() {
  ...
  body.querySelectorAll('.card[data-sprite]').forEach(el => {
    el.addEventListener('click', () => openModal(el.dataset.sprite, el.dataset.variant));
  });
}
